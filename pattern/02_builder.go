...
Строитель — это порождающий паттерн проектирования, который позволяет пошагово создавать объекты. Когда создаётся сложный объект,
у конструктора может быть слишком много параметров, причём часть из них используются редко. Вместо этого можно определить несколько
методов-конструкторов, которые будут отвечать за инициализацию определённых параметров. Тогда сложный объект будет создаваться в
несколько шагов.
...
- операции, где необходима постеменная настройка/создание объекта
